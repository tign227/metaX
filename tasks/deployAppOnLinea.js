...
task("metaX:deployOnLinea", "deploy all contracts of metaX application on linea").setAction(
  async (taskArgs, hre) => {
    ...
    await hre.run("configMechPetOnLinea", { mechPetAddress });
...
//     await hre.run("configPriceFeedOnLinea", { priceFeedAddress });
    ...
    toJson(PATHS.ADDRESS, json, `metaX.linea.${network}.json`);
  }
);

subtask("configMechPetOnLinea", "config mech pet mapping")
...
subtask("configPriceFeedOnLinea", "config price feed")
